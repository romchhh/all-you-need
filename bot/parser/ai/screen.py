...
ЗАДАЧА:
1. accept=false если пост НЕ является объявлением о ТОВАРЕ или УСЛУГЕ. Жёстко отсекай:
   - новости, обсуждения, опросы, мемы, чат без оффера
   - рекламу канала/бота, правила группы, поздравления, репосты без продажи
   - ВАКАНСИИ / поиск работы / подработка / «набор в команду» — это НЕ товар и НЕ услуга мастера
   - схемы «есть вариант заработать», «от 150$ в сутки», «пиши в бота @…vakansiy…»
   - набор людей на удалёнку / MLM / «лёгкий заработок» без продажи конкретного товара
   - «куплю / шукаю / ищу» без предложения своего товара или услуги на продажу
   - спам, пустые посты, мусор, несвязанный контент
   ВАЖНО: если пост только зовёт писать в бота про работу/заработок — accept=false, reject_reason=job|spam.
...
3. accept=true ТОЛЬКО если есть конкретное предложение:
   - товар (продажа / отдам / обмен) с понятным предметом ИЛИ
   - услуга мастера клиенту (маникюр, ремонт, уборка, перевозка вещи, репетиторство и т.п.)
   НЕ accept: вакансии, заработок «в сутки», рефералы в ботов вакансий — даже если есть «цена».
4. Если accept=true — улучши поля:
   ...
   - category/subcategory: ТОЛЬКО ты по смыслу товара во всём тексте (не копируй подсказку парсера).
     ...
     iPhone → electronics/smartphones; услуги мастера → services_work + подкатегория.
     НИКОГДА не принимай вакансии через category=services_work/vacancies — такие посты reject (job).
