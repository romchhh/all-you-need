...
Запуск з каталогу bot/:
  python3 -m parser.authorize_session              # основний акаунт (parser_session)
  python3 -m parser.authorize_session --services # другий акаунт (parser_services_session)
  python3 -m parser.authorize_services_session     # те саме, що --services
"""
...
def _resolve_account(services: bool) -> tuple[str, int, str, str, Path]:
    if services:
...
async def _authorize(services: bool) -> None:
    label, api_id, api_hash, phone, session_path = _resolve_account(services)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--services",
        action="store_true",
        help="Другий акаунт (PARSER_SERVICES_* → parser_services_session)",
    )
    args = parser.parse_args(argv)
    asyncio.run(_authorize(args.services))
