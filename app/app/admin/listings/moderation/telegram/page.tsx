...
export default function TelegramModerationPage() {
  ...
  const fetchListings = async () => {
    try {
      const response = await fetch('/api/admin/moderation/telegram?status=pending');
      
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
        console.error('Failed to fetch listings:', errorData.error);
        setListings([]);
        return;
      }
  ...
  const handleApprove = async (listing: Listing) => {
    ...
    try {
      const response = await fetch('/api/admin/moderation/telegram', {
        method: 'POST',
  ...
  // Для Telegram оголошень images містить file_id з Telegram, які не можна відобразити напряму
  // Потрібно використовувати Telegram Bot API для отримання фото
  // Поки що показуємо placeholder
  const getImageUrl = (images: string) => {
    // Telegram file_id не можна використовувати як URL
    // Тут можна додати API endpoint для отримання фото через Telegram Bot API
    ...
    <div className="p-6">
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">📱 Модерація оголошень з Telegram бота</h1>
            <p className="text-gray-600">Оголошень на модерації: {listings.length}</p>
          </div>
          <div className="flex gap-3">
            <Link
              href="/admin/listings/moderation"
      ...
      {listings.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <p className="text-gray-600">Всі оголошення з Telegram бота перевірено</p>
        </div>
      ) : (
              ...
              <div className="p-6">
                <div className="flex gap-6">
                  ...
                  <div className="flex-shrink-0">
                    <div className="w-48 h-48 bg-gray-100 rounded-lg flex flex-col items-center justify-center border-2 border-dashed border-gray-300">
                      <span className="text-4xl mb-2">📷</span>
                      <span className="text-gray-400 text-xs text-center px-2">Telegram фото<br/>(file_id)</span>
                    </div>
                  </div>
