...
export default function MarketplaceModerationPage() {
  ...
  const fetchListings = async () => {
    try {
      const response = await fetch('/api/admin/moderation/marketplace?status=pending');
  ...
  const handleApprove = async (listing: Listing) => {
    ...
    try {
      const response = await fetch('/api/admin/moderation/marketplace', {
        method: 'POST',
    ...
    <div className="p-6">
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">🌐 Модерація оголошень з маркетплейсу</h1>
      ...
      {listings.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <p className="text-gray-600">Всі оголошення з маркетплейсу перевірено</p>
        </div>
      ) : (
