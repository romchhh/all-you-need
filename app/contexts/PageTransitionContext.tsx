...
/** Мінімальний час показу лоадера при відкритті (мс). */
...
function PageTransitionOverlay({ visible }: { visible: boolean }) {
  ...
  return createPortal(
    <div
      className={`fixed inset-0 z-[3000] flex items-center justify-center transition-opacity duration-150 ${
        isLight ? 'bg-white/80' : 'bg-black/75'
      }`}
      role="status"
      aria-live="polite"
      aria-busy="true"
    ...
    >
      <div className="flex flex-col items-center gap-3 px-6">
        <div className="flex h-16 w-16 items-center justify-center animate-tg-logo-spin">
          ...
          <img
            ...
            width={408}
            height={129}
            className="h-11 w-auto max-w-[9rem] object-contain select-none pointer-events-none"
            draggable={false}
          />
        </div>
        <p
