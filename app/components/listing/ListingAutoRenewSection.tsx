...
import { useListingAutoRenew } from '@/features/listing/hooks/useListingAutoRenew';
import { useLanguage } from '@/contexts/LanguageContext';
import { getAppearanceClasses } from '@/utils/appearanceClasses';
import type { TelegramWebApp } from '@/types/telegram';
import type { ToastType } from '@/features/ui/hooks/useToast';
...
type ListingAutoRenewSectionProps = {
  ...
  /** Вузький рядок (картка в профілі): чекбокс + короткий підпис в один ряд. */
  compact?: boolean;
};
...
export function ListingAutoRenewSection({
  ...
  const { autoRenewLocal, autoRenewSaving, persistAutoRenew } = useListingAutoRenew({
    listingId,
    serverAutoRenew,
    onPersistSuccess,
  });
      ...
      <div className={shell}>
        <label className="flex cursor-pointer select-none items-center gap-2">
          <input
            ...
            className={`h-3.5 w-3.5 shrink-0 rounded border accent-[#3F5331] focus:ring-2 focus:ring-[#3F5331]/30 ${
          ...
          />
          <span className={`min-w-0 flex-1 text-xs font-medium leading-tight ${ac.pageHeading}`}>
            {t('sales.autoRenewProfileRow')}
          </span>
          {autoRenewSaving ? (
            <Loader2 className={`h-3.5 w-3.5 shrink-0 animate-spin ${ac.mutedText}`} aria-hidden />
          ) : null}
        </label>
      </div>
    );
